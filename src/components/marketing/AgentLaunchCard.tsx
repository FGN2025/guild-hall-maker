  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [ev, tr] = await Promise.all([
        // tenant_events stores the label in `name` and the date in `start_date`;
        // tournaments are platform-wide and carry no tenant_id column, so they
        // are not tenant-filtered here.
        supabase.from("tenant_events" as any).select("id, name, start_date").eq("tenant_id", tenantId).gte("start_date", now).order("start_date").limit(30),
