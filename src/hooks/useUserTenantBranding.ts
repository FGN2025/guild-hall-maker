 ...
 * Returns null when the user has no tenant affiliation. Respects the
 * banner page's publish_at / unpublish_at scheduling window.
 */
export const useUserTenantBranding = () => {
  ...
  return useQuery<TenantBranding | null>({
    ...
    queryFn: async () => {
      ...
      const withinWindow =
        !!bannerPage?.id &&
        (!bannerPage.publish_at || new Date(bannerPage.publish_at).getTime() <= now) &&
        (!bannerPage.unpublish_at || new Date(bannerPage.unpublish_at).getTime() > now);
