...
const sectionData: { id: string; icon: typeof Shield; title: string; bullets: string[] }[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "— SHOCKBYTE HOSTING SETUP GUIDE —",
      "Shockbyte is a popular game-server hosting provider that supports American Truck Simulator, Minecraft, ARK, and 40+ other titles.",
      "Step 1 — Purchase & Provision: Visit shockbyte.com, choose your game, select a plan (player slots & region), and complete checkout. Your server is provisioned automatically within a few minutes.",
      "Step 2 — Find Your Connection Details: Log in to the Shockbyte control panel at panel.shockbyte.com. On the main Console page, your Server IP and Port are displayed prominently at the top (e.g. 157.85.92.29:27015). The format is IP:Port.",
      "Step 3 — Add to Platform: In Admin → Game Servers, click 'Add Server'. Enter the server name (e.g. 'ATS Community Server'), select the matching game from the dropdown, then enter the IP address and port from Shockbyte as separate fields — IP goes in the 'IP Address' field, port number goes in the 'Port' field.",
      "Step 4 — Server Configuration: In the Shockbyte panel, use the 'Configuration' or 'Startup' tabs to set your server name, password, max players, mods, and game-specific settings. These settings are managed in Shockbyte, not on this platform.",
      "Step 5 — Connection Instructions: Use the 'Connection Instructions' field (or hit 'Enhance' for AI-generated text) to tell players exactly how to join. Example for ATS: 'Open American Truck Simulator → Multiplayer → Direct Connect → Enter 157.85.92.29:27015 → Click Connect.'",
      "Step 6 — FTP / File Access: Shockbyte provides SFTP access for uploading mods, maps, and config files. Find SFTP credentials under the 'Settings' tab in the Shockbyte panel. Use an FTP client like FileZilla to connect.",
      "Step 7 — Subdomain (Optional): Shockbyte allows you to create a free subdomain (e.g. myserver.shockbyte.com) under 'Network' settings. You can use this subdomain as the IP Address in the platform instead of the numeric IP for a friendlier display.",
      "Step 8 — Pterodactyl Panel Integration: Shockbyte uses the Pterodactyl panel. To enable live status on this platform, toggle 'Pterodactyl Panel' in the server form, enter your Shockbyte panel URL (https://panel.shockbyte.com) and the Server ID (found in the URL when viewing your server, e.g. panel.shockbyte.com/server/abc123 → the ID is abc123). Note: A Pterodactyl API key must be configured in your platform secrets for status checks to work.",
      "Troubleshooting — If players can't connect: verify the server is running in the Shockbyte console, confirm the IP and port match exactly, check that the game version matches between server and client, and ensure any required mods are installed on both sides.",
    ],
  },
  {
    id: "seasons",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Health Check Endpoint — External ecosystem apps (e.g., FGN Academy) can poll the health endpoint to verify play.fgn.gg's API, database connectivity, and secret configuration status without transferring user data.",
      "FGN Academy Integration — Challenge completions are automatically synced to FGN Academy's Skill Passport using a flat JSON payload (user_email, challenge_id, score, task_progress, skills_verified). Authentication uses the X-Ecosystem-Key header backed by the ECOSYSTEM_API_KEY secret (legacy X-App-Key/FGN_ACADEMY_API_KEY retired in P-3).",
      "Academy Sync Contract — The integration guide is versioned at docs/play-fgn-gg-integration-guide.md. Score is normalized as round((awarded_points / max_points) × 100). A score ≥ 70 is treated as a pass on the academy side.",
      "Academy Data API — The ecosystem-data-api edge function exposes enriched challenge data (game names, durations, nested tasks) so the academy can import challenges and create work orders.",
      "Sync Monitoring — All sync attempts are logged in the ecosystem_sync_log table with target_app, data_type, status, records_synced, and error_message for auditing.",
    ],
  },
  {
  ...
  },
  {
    ...
    bullets: [
      ...
      "Academy Sync Status — Each completion record tracks academy_synced (boolean), academy_synced_at (timestamp), and academy_sync_note (reason text). If the player isn't registered on the academy, the note is set to 'user_not_found' and a 'Join FGN Academy' banner appears on their challenge detail page.",
      "Retry Academy Sync — Admins and moderators can manually re-trigger the academy sync for any completion using the 'Retry Academy Sync' button in the approval flow.",
      "Sync Feedback — Real-time toast notifications surface the sync result during approval: success confirmation, 'player not registered' warning, or error details.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Email Notifications — Redemption status changes, new challenges, and points adjustments also trigger email notifications via the notification system.",
      "Automatic Triggers — All 8 notification types are generated by database triggers (or scheduled jobs), requiring no manual action from moderators beyond their normal workflows.",
      "Player Preferences — Players can independently toggle In-App and Email channels for each notification type (including Points Adjustments) from their Profile Settings page. All channels default to enabled.",
  ...
  },
  {
    ...
    bullets: [
      "Dark Mode Default — The Arcade experience now boots in Dark mode on every load. The brand experience requires it, so the theme is reset to Dark on each visit and is no longer persisted across sessions.",
      "Toggle — A Theme button in the sidebar footer lets a player switch to Light for the current session only. The 'System' option has been removed.",
      "Enterprise Mode — Pages rendered with `?mode=enterprise` or inside an iframe still use the Enterprise (light) palette. This is intentional and unrelated to the player-facing toggle.",
      ...
      "UI Readability — On pages with rich backgrounds, components use `bg-black/40` frosted-glass surfaces for high white-text contrast (see UI Readability memory). No theme toggle is required for legibility.",
      "Cookie Consent — A cookie consent banner appears on first visit. Players can accept or decline; the choice is persisted.",
    ],
  },
  {
    id: "ai-game-guide-writer",
    icon: Sparkles,
    title: "AI Game Guide Writer",
    bullets: [
      "Where — Admin → Games → click 'AI Writer' on any game card. Admins only.",
      "Three Modes — 'Generate' writes a fresh User Guide from scratch. 'Enrich' improves the existing draft while preserving structure. 'Section' rewrites a single named section (Overview, Controls, Tips & Strategy, etc.) and returns the full guide with that section integrated.",
      "Notebook-Grounded — When the game is mapped to an Open Notebook connection (Admin → Notebooks), the writer uses RAG to pull excerpts from the notebook so the output reflects the source of truth, not invented content.",
      "Output Format — The writer returns GitHub-flavored Markdown. The platform renders this as styled HTML on the public Game Detail page (headings, bullets, code, bold, etc.).",
      "Rendered vs Markdown Preview — The preview area in the dialog has two tabs: 'Rendered' (1:1 with the player view, default) and 'Markdown' (the editable source). Admins should review Rendered before clicking Apply.",
      "Apply to User Guide — The Markdown source is saved to `games.guide_content`. The change appears immediately on the public /games/:slug page.",
      "Edge Function — Powered by `supabase/functions/generate-game-guide` using `google/gemini-2.5-pro` via the Lovable AI Gateway. Admin role is enforced server-side.",
      "Tip: Use 'Generate' for new games, 'Enrich' to fill gaps once a draft exists, and 'Section' for surgical updates without disturbing the rest of the guide.",
    ],
  },
  {
    id: "tenant-self-service",
    icon: UserPlus,
    title: "Tenant Self-Service Signup",
    bullets: [
      "Public Page — Providers can self-register at `/for-providers`. The page collects organization name, contact email, admin name, and password.",
      "Provisioning — The `provision-tenant` edge function creates the auth user, generates a unique slug, inserts the tenant row with status 'provisioning', assigns the `tenant_admin` role, and creates a Stripe Checkout session.",
      "Activation — On successful Stripe checkout for the Tenant Basic plan, the webhook upserts the subscription row AND flips the tenant status from 'provisioning' to 'active'. Until then, the tenant cannot use their portal.",
      "Lead Capture — Inquiries from the same `/for-providers` page (without checkout) are captured in the inquiry inbox at Admin → Inquiries for follow-up.",
      "Admin Override — Platform Admins can manually flip a stuck 'provisioning' tenant to 'active' from Admin → Tenants if a webhook is delayed or fails.",
    ],
  },
  {
    id: "onboarding-bypasses",
    icon: Shield,
    title: "Onboarding & Identity Gates (Bypass Rules)",
    bullets: [
      "Three Gates — Players go through (1) email confirmation, (2) ZIP / tenant matching or bypass code, (3) Discord verification, before the platform unlocks.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Edge functions exposed to anon traffic must set `verify_jwt=false` and validate input themselves.",
    ],
  },
  {
    id: "legal-pages",
  ...
  },
  {
    ...
    icon: Gamepad2,
    title: "Steam Integration Dashboard",
    bullets: [
      "Where — /admin/steam.",
      "Purpose — Single pane of glass for everything Steam-related on the platform.",
      "Linked Players — Lists every player who has linked their Steam account (steam_id, steam_username, link timestamp). Useful for verifying account hygiene.",
      "Steam-Enabled Games — Lists every game in the catalog with a steam_app_id set, alongside the count of linked players who own that game. Use this to identify which catalog entries can support Steam auto-verified challenges.",
      "Auto-Verified Evidence — Shows recent challenge evidence approvals that were auto-resolved by querying Steam (no moderator review). Helpful for auditing how often the integration is firing.",
      "Tasks Using Steam Verification — Lists challenge tasks with verification_type set to steam_achievement or steam_playtime, with the target achievement_id or minimum_playtime_minutes alongside.",
      "How Auto-Verify Works — When a player enrolls in a challenge with a Steam-verified task, the verify-steam-task-completion edge function checks their Steam profile against the criterion. Match → auto-approve and award points. No match → the task remains pending until the criterion is met.",
      "Configuration — Steam Web API access requires STEAM_API_KEY in project secrets. The Steam OpenID callback is handled by the steam-openid-callback edge function.",
      "Tip: Encourage players to link Steam in Profile Settings — it unlocks auto-verified tasks and powers the achievement sync.",
    ],
  },
  {
    id: "points-rubric",
  ...
  },
  {
    ...
    bullets: [
      "Where — /admin/calendar-images.",
      "Purpose — Upload a full-bleed seasonal hero image for each month of the year that renders across the public /calendar page.",
      "Upload — Pick a month, upload an image (PNG/JPEG/WebP). Image is stored in a dedicated calendar-images storage bucket and referenced from calendar_monthly_images.",
      "Replace — Re-uploading for a month replaces the existing image. Previous images are deleted from storage to keep the bucket clean.",
      "Display — The image renders behind the month grid on /calendar with the bracket info and event chips overlaid. Players see a different banner each month.",
      "Tip: Use images that work as horizontal banners — wide aspect ratios (16:9 or wider) render best behind the calendar grid.",
    ],
  },
  {
    id: "cdl-generator",
    ...
    bullets: [
      "Where — /admin/challenges/generate (moderators access the same wizard at /moderator/challenges/generate).",
      "Purpose — AI-generate compliant CDL / ATS career-path challenges that score 18/18 on the platform's CDL benchmark for completeness, structure, and CFR alignment.",
      "Step 1: Configure — Pick a CDL domain (Pre-Trip Inspection, Vehicle Control, Hours of Service, etc.), enter the CFR reference, choose difficulty, and select the game (typically American Truck Simulator).",
      "Step 2: Generate — Click Generate. The generate-cdl-challenge edge function calls the AI model, structures the response into challenge + tasks + evidence requirements, and runs the 18-point validator. The UI shows the score and any missing items.",
      "Step 3: Review & Edit — Inline-edit the title, description, learning objectives, tasks, and evidence rules. Re-run the validator after edits to confirm 18/18.",
      "Step 4: Publish — One-click publish via the publish-cdl-challenge edge function, which inserts the challenge with cdl_domain, cfr_reference, skill_tags, and certification_description populated. Inactive by default.",
      "Activation — Generated challenges are inactive so you can preview at /challenges/:id before exposing to players. Flip the active toggle from Admin → Challenges when ready.",
      "FGN Academy Sync — Once a player completes a published CDL challenge, it syncs to FGN Academy with the appropriate career-path mapping for tenants with Academy sync enabled.",
    ],
  },
  {
    ...
    bullets: [
      "Where — /admin/inquiries.",
      "Purpose — Review and triage inbound contact form submissions from the public /for-providers lead gen page.",
      "What's Captured — Each inquiry stores name, email, organization, role, message, and submission timestamp from the provider_inquiries table.",
      "Filtering — Filter by role (Owner, Marketing, IT, Operations, Other) to prioritize the buyers who can sign a Stripe trial.",
      "Delete — Remove spam or completed inquiries permanently after responding.",
      "Workflow — Inquiry arrives → review and respond by email manually → delete from the queue. The system does NOT auto-send a confirmation email to the inquirer (intentional — let staff personalize the first touch).",
      "Tip: Pair this with the Tenant Self-Service signup flow. Inquiries that don't self-serve via /for-providers Stripe checkout land here for a manual follow-up.",
    ],
  },
  {
    id: "discord-webhooks",
    icon: Plug,
    title: "Discord Webhook Infrastructure",
    bullets: [
      "Where — Admin → Ecosystem → Discord.",
      "Purpose — Auto-post platform events to Discord channels via webhook URLs without running a bot.",
      "Tables — discord_channel_webhooks stores the webhook URL per purpose (e.g., tournament_published, tournament_completed, tenant_event_published) and optional tenant_id for tenant-scoped routes. discord_send_log records every send attempt with status, response, and error.",
      "Auto-Fire Triggers — Database triggers fire automatically when: a tournament status changes to 'in_progress', a tournament status changes to 'completed', or a tenant event is published. The trigger calls the discord-send-message edge function with the configured webhook.",
      "Configuration — Add a webhook URL by purpose (global) or per-tenant. Tenant-scoped webhooks override global ones for that tenant's events.",
      "Send Log — Use discord_send_log to audit which messages went out, retry failed sends, and diagnose channel permission issues. The log is admin-only via RLS.",
      "Setting Up a Channel Webhook — In Discord: Server Settings → Integrations → Webhooks → New Webhook → pick channel → copy URL. In Admin → Ecosystem → Discord: paste the URL and select the purpose.",
      "Daily Backlog Reminder — Outstanding Discord setup tasks are tracked in the .lovable/backlog/discord.md file and emailed daily to darcy@fgn.gg via send-discord-backlog-reminder at 8 AM Eastern.",
      "Tip: Use the same channel for multiple purposes by registering the same webhook URL against each purpose, or split signal types into dedicated channels for cleaner discovery.",
    ],
  },
  {
    id: "staff-emails",
    ...
    bullets: [
      ...
      "Disabling — To stop a reminder permanently, ask the agent to disable the corresponding scheduled function and remove the cron job. Don't disable Lovable Emails wholesale — that breaks auth and other app emails too.",
      "Adding Recipients — Templates currently have darcy@fgn.gg baked in via the template 'to' field. Adding additional recipients requires editing the template registry entry or adding additional invoke calls in the scheduled function.",
    ],
  },
  {
    id: "bulk-register-legacy",
    icon: Database,
    title: "Bulk Register Legacy Users",
    bullets: [
      "Where — Admin-only edge function bulk-register-legacy-users (no dedicated UI yet — invoke via admin console or curl).",
      "Purpose — Bulk-create Supabase auth accounts for legacy_users records that have an email but no associated auth user, so historical players can claim their record by signing up with the same email.",
      "Inputs — dry_run (boolean, default true) and batch_size (integer, default 50). Always run with dry_run: true first to inspect what would be created.",
      "Output — Returns a summary { processed, created, skipped, errors[] } so you can confirm the batch executed cleanly.",
      "Safety — The function checks for existing auth users by email before creating, so re-running is safe. Skipped rows include the reason (already_exists, missing_email, invalid_email).",
      "Auto-Claim — Once an auth account is created, the legacy record auto-claims when the user signs in (matched via lower-cased email) — no further action required.",
      "When to Use — After a CSV import of legacy users, before a marketing campaign that asks legacy customers to log in, or after a billing-system sync that exposes new email addresses.",
