...
const sectionData: { id: string; icon: typeof Scroll; title: string; bullets: string[] }[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Evidence Upload — Upload screenshots, videos, or files as proof of completing each task. YouTube and Twitch links render inline for the reviewer.",
      ...
      "Edit Window — While evidence is Pending or Rejected, you can delete and re-upload freely.",
      "Locks on Submission — Once you submit a task for review, that upload is locked. Once a moderator Approves it, the lock is permanent — approved evidence cannot be deleted or replaced. This protects your XP and point payouts.",
      "Unenrollment Lock — Submitting any evidence permanently disables the Unenroll button on this quest. If you genuinely need out, ask a moderator.",
      "Moderator Review — A moderator reviews each piece of evidence and may leave feedback notes. Re-upload only if the status is Rejected.",
    ],
  },
  {
    id: "per-task-points",
    icon: Zap,
    title: "Per-Task Point Payouts (Detailed)",
    bullets: [
      "Pay-As-You-Go — Quest points are paid out per task at the moment of approval, not in one lump sum at the end. Approve task 1 → points hit your wallet immediately. Approve task 2 → another payout. And so on.",
      "Points Per Task — Each approved task earns the quest's 'points per task' value. A 5-task quest at 2 points per task = up to 10 total, paid in 5 increments of 2.",
      "Where to See It — On the quest detail sidebar, look for '+X per task' next to the total possible (X × task count). This is your projected ceiling assuming every task is approved.",
      "Both Wallets Move — Each per-task payout adds to both your lifetime 'Points' (leaderboard) and your spendable 'Points Available' (Prize Shop).",
      "Instant Notification — Each approval triggers an in-app notification ('Points Awarded — Quest Name — Task X — +N pts') so there's no guessing about what was paid.",
      "No Double-Paying — The payout trigger is idempotent: each task can only award points once per enrollment. Re-approving a task doesn't double-pay.",
      "Quest Completion ≠ Final Payout — When all tasks are approved and the moderator marks the whole quest 'completed', that triggers chain progression and achievement checks — but the points are already in your wallet from the per-task approvals.",
      "Rejected Tasks — Rejection does not deduct points (you never got paid for a rejected task). Re-upload, get approved, and the payout fires then.",
    ],
  },
  {
    id: "academy-credit",
  ...
  return (
    <div className="relative">
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 relative z-10">
        ...
        <QuickReferenceCard audience="quest" intro="XP ranks, achievement tiers, points economy, and the quest enrollment flow." />
