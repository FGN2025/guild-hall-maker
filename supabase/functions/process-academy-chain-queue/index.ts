// Drain the academy_chain_sync queue. Mirrors the task/quest workers.
...
const QUEUE = "academy_chain_sync";
const DLQ = "academy_chain_sync_dlq";
const BATCH = 25;
...
Deno.serve(async (req) => {
  ...
  for (const m of (msgs || []) as Array<{ msg_id: number; read_ct: number; message: any }>) {
    ...
    const { user_id, chain_id } = payload;
    ...
    let success = false;
    try {
      const { data, error } = await admin.functions.invoke("sync-quest-chain-to-academy", {
        body: { user_id, chain_id },
      });
      if (error) console.error("sync-quest-chain-to-academy invoke error:", error.message);
      else success = data?.success === true;
    } catch (e) {
      console.error("sync-quest-chain-to-academy threw:", (e as Error).message);
    }
  ...
  console.log("academy_chain_sync drain:", JSON.stringify(summary));

  if (processed > 0) {
    await admin.from("ecosystem_sync_log").insert({
      data_type: "academy_chain_sync_worker",
